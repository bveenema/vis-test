...
/***/ function(module, exports, __webpack_require__) {
	...
	var svg = d3.select("#viz-wrapper").append('svg').attr('height', height + 2 * padding).attr('width', width + 2 * padding);
	...
	// Setup the trendline
	var trendline = d3.line().x(function (d) {
	...
	var dataDrag = d3.drag().on('start', dragStart).on('drag', dragging).on('end', dragEnd);

	var wasDraggedFLAG = false;
	...
	d3.json('mock/data.json', function (error, data) {
		...
		// Setup the domains
		var xDomain = padTimeScale(d3.extent(data, function (data) {
			return d3.isoParse(data.date);
		}), 1);

		var yDomain = padDataScale(d3.extent(data, function (data) {
			return data.value;
		}), 1);
		...
		// Add the trendline
		viz.append('path').data([data]).attr('class', 'trendline').attr('d', trendline).style('fill', 'none').style('stroke', 'black');

		// Add the dots
		var dataPoints = viz.selectAll('g.data-point').data(data).enter().append('g').attr('class', 'data-point');

		dataPoints.attr('transform', function (d) {
			var x = xScale(d3.isoParse(d.date));
			var y = yScale(d.value);
			return 'translate(' + x + ',' + y + ')';
		}).style('stroke', 'none').style('fill', 'blue');

		dataPoints.append('circle').attr('r', defaultRadius);

		var tooltip = dataPoints.append('g').attr('class', 'tooltip hide').attr('transform', 'translate(5,5)');
		tooltip.append('text').text(function (d) {
			return d.value;
		});

		// Display text on hover
		dataPoints.on("mouseenter", function (d, i) {
			d3.select(this).select('g.tooltip').classed('show', true).classed('hide', false);
		});

		dataPoints.on("mouseleave", function (d, i) {
			d3.select(this).select('g.tooltip').classed('show', false).classed('hide', true);
		});

		// Add click event handlers
		svg.on("click", function () {
			console.log('viz clicked');
			d3.selectAll('g.data-point').select('circle').classed('selected', false);
		...
		dataPoints.on("click", function (d, i) {
			...
			d3.select("#updator").attr('class', 'show').attr('dataPoint', i);
			d3.select("#value").attr('placeholder', d.value);
		});

		// Add #updator submit handler
		d3.select('#updator').on('submit', function () {
			...
			var i = parseInt(d3.select(this).attr('dataPoint'));
			data[i].value = newValue;
			var newPositionY = yScale(data[i].value);
			d3.select(dataPoints._groups[0][i]).attr("transform", 'translate(436,' + newPositionY + ')');

			//recalculate trendline
			d3.select('.trendline').attr('d', trendline);
	...
	function dragStart(d) {
		...
		d3.select(this).select('circle').attr('r', defaultRadius * 1.15);
	};
	...
	function dragEnd(d) {
		d3.select(this).select('circle').attr('r', defaultRadius);
	...
	var padTimeScale = function padTimeScale(domain, padding) {
		if (domain.length !== 2) {
			return;
		}
	...
	//does opposite for max value
	var padDataScale = function padDataScale(domain, padding) {
		if (domain.length !== 2) {
			return;
		}
